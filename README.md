...
1. 打开刚创建的新页面
2. 右上角「分享」→「添加协作者」
3. 搜索框输入你的应用名称（在飞书开放平台里给应用起的名字）
4. 选中带橙色「应用」标签的那个 → 权限选「**可编辑**」→ 确认

> ⚠ 常见问题 1：搜不到应用 → 检查飞书开放平台里应用是否已**发布**，未发布的应用不会出现在搜索结果里。
>
> ⚠ 常见问题 2：加了协作者但 API 仍返回 403 → 需要对**这个具体页面**授权，在父页面授权不会自动继承到子页面。
>
> ⚠ 常见问题 3：一个知识库内多个页面都要用同一个应用 → 在知识库设置的「成员管理」里加一次，之后该知识库所有新建页面自动继承。
